...
- The `GenUI` ID is reserved for GenUI's own panel; another Controllable asking for it is refused and left unregistered.
- The sample prefab starts with its UI hidden; press F1 to show it.
...
- Dragging a numeric member's label in the browser scrubs its value, at the rate and with the modifiers the panel uses.
- Tab and Shift + Tab walk the browser's editable fields once the focus is in one of them.
...
- Right-clicking a row in the browser shows its OSC control address, selected and ready to copy.
