...
public class Vector3IntUI : ControllableUI
{
    ...
    public void CreateUI(Controllable target, FieldInfo property, bool isInteractible)
    {
        ...
        ZInput.contentType = InputField.ContentType.IntegerNumber;

        var scriptValue = (Vector3Int)property.GetValue(target);

        XInput.text = scriptValue.x.ToString();
        YInput.text = scriptValue.y.ToString();
        ZInput.text = scriptValue.z.ToString();
    ...
    { 
        ...
        var vector = (Vector3Int)Property.GetValue(LinkedControllable);
		...
		this.transform.GetChild(0).Find("ZInput").GetChild(0).GetComponent<InputField>().text = vector.z.ToString();
	}
}
